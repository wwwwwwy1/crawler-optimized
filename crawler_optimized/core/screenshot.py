...
def capture(serial: str, activity: str, fingerprint: str, segment_index: int = 0) -> str | None:
    ...
    layout_hash = fingerprint.rsplit("|", 1)[-1][:12]
    filename = (
        f"{_safe_name(activity)}_{layout_hash}_s{segment_index}_{timestamp}.png"
    )
    ...
    image = _crop_image(image, serial)
    if image is None or not _quality_check_image(image):
        return None
...
def _crop_system_bars(filepath: str, serial: str) -> bool:
    ...
    if img is None:
        return False
    cropped = _crop_image(img, serial)
    if cropped is None:
        return False
    return bool(cv2.imwrite(filepath, cropped))


def _crop_image(image: np.ndarray, serial: str) -> np.ndarray | None:
    """在内存中裁掉系统状态栏和导航栏。"""
    h, w = image.shape[:2]
...
def _quality_check(filepath: str) -> bool:
    """基本质量检查: 非纯色、分辨率达标"""
    img = cv2.imread(filepath)
    return img is not None and _quality_check_image(img)


def _quality_check_image(img: np.ndarray) -> bool:
    """对内存图像执行基础质量检查。"""
    h, w = img.shape[:2]
    if w < 720 or h < 1000:
        return False
